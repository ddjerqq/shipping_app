...
internal sealed class RegisterCommandHandler(IAppDbContext dbContext) : IRequestHandler<RegisterCommand, User>
{
    public async Task<User> Handle(RegisterCommand request, CancellationToken ct)
    {
        ...
        var user = new User(UserId.New())
        {
            ...
            CultureInfo = request.CultureInfo ?? new CultureInfo("en-US"),
            TimeZone = request.TimeZoneInfo ?? TimeZoneInfo.Utc,
        };
